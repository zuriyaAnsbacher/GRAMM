import copy
from GR_code.GG_GRAMM.code.aux_functions_oldVersion import common_chr, who_fuller, high_res, equal_al
...
def data_pc_22(al_p, al_c, j, fm_in, fm_out, types):
    ...
    if match == 1:  # 1 pair same, 1 pair different
        ...
        al_p[ind_p] = high_res(al_p[ind_p], al_c[ind_c])  # emb chromo with high res between par and _child_
        al_p2 = Als()
        ...
        c2_outpar = fm_in[1 - j][types] if fm_in[1 - j] else None  # the second chromo in other par (that _child_ inheritance)
        # try
...
def add_data_child(child, chF, chM, emb_FM, al_types, d_chi):
    f_in, f_out, m_in, m_out = common_chr(chF, chM, emb_FM)  # f_in/m_in = chromos the _child_ inheritance
    fm_in = [f_in, m_in]
    fm_out = [f_out, m_out]
    ...
    for chr_par in fm_in:
        ...
        for types in al_types:
            ...
            if al_p.is_empty_a():
                ...
                chr_par[types] = copy.deepcopy(al_c)  # emb data from _child_ to par
            elif len(al_c) == len(al_p) == 1:  # par and _child_ has 1 chromo
                if al_c != al_p:
                    return None, None
                al_p[0] = high_res(al_p[0], al_c[0])  # emb chromo with high res between par and _child_
                ...
                al_p[ind_p] = high_res(al_p[ind_p], al_c[0])  # high res  # TODO: why not chr_par[alleles_names] = ... ?
                al_p2 = Als()
                al_p2.append(al_p[1 - ind_p])
