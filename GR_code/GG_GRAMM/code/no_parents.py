...
def create_merged_allele_values(children, al_name):
    ...
    for example: if child 1 has A : [02:01, 03:04], child 2: A: [05, 03], child 3: A: [02, 07],
...
def check_num_child_with_diff_values_in_specific_allele(children, al_name):
    ...
    for example, child 1: A[01, 02], child 2: A[03, 04], child 3: A[01, 02]
...
def insert_data_of_2_children_no_homozygous(dict_children_one_allele, hapF, hapM, al_name):
    ...
    child 1 : [X1, X2], child 2: [Y1, Y2] --> hapF.hap1 = [X1], hapM.hap1 = [X2]; hapF.hap2 = hapM.hap2 = [Y1, Y2]
    :param dict_children_one_allele: children dict, that contains data about one allele only
...
def insert_data_of_2_children_one_homozygous(dict_children_one_allele, hapF, hapM, al_name):
    ...
    for example: child 1: [01, 01], child 2: [02, 03] so the parents are 01+02, 01+03
    """
    child1_al1, child1_al2, child2_al1, child2_al2 = get_4_alleles_from_2_children(dict_children_one_allele)
...
def divide_2_alleles_to_non_empty_groups(gr1, gr2, alleles, try_again):
    """
    check the intersection between g1, g2 (that have at least one allele in each of them) and the alleles.
    if no intersection (g1:[01], g2:[02], alleles:[03, 04]: we cannot add data to groups, so we save the alleles in the
    list 'try_again', and we call to this function again with those alleles (in hope that in the second time, after
    adding more information, will be intersection.)
    if 1 intersection (g1:[01], g2[02], alleles:[01, 03]: so we add the allele who not the intersection ('03'), to the
    group that do not have the intersection (g2)
    if 2 intersections: it means that there is not new information to gr1 and gr2
    :param gr1: group 1
    :param gr2: group 2
    :param alleles: alleles pair
...
def divide_alleles_to_2_groups(dict_children_one_allele):
    """
    dividing alleles of 3 children or more to 2 groups (one for each parent)
    for example, c1:[01, 02], c2:[02, 03], c3:[01, 04], so -> par1: 01~03, par2: 02~04
    there are 2 cases:
        1. easy case: there is homozygous child, so divide his alleles to the 2 groups, and then go over the other
    ...
    if is_homoz:  # if a child has [01, 01] so each parent has '01'
...
def insert_data_to_single_allele_in_par_haps(family, alleles_names, hapF, hapM, count_fam, errors_in_families):
    ...
    for al_name in alleles_names:
        ...
        if len(merged_allele_values) == 4:
            ...
            # or an homozygous child (could be also the 2 children).
            ...
            if num_children == 2:
                ...
                elif len(set(merged_allele_values)) < 4:  # there is homozygous child
                    insert_data_of_2_children_one_homozygous(dict_children_one_allele, hapF, hapM, al_name)
                success = True
                allele_inserted_to_parents = al_name
                break  # we do not need to check more alleles, if we succeeded.
...
def associate_children_to_par_haps_while_parents_dont_exist(hapF, hapM, child, allele_inserted):
    """
    associate child to the parents haplotypes he inherited. this function is for scenario of no parents in data.
    ...
    4. one value in child has 1 match, and the second: 2 matches. it could happened in one of the following options:
